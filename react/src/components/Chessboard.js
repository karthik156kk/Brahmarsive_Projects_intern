...
function Chessboard() {
    ...
    for(let i=VERTICAL_AXIS.length-1; i>=0; i--){
        for(let j=0; j<HORIZONTAL_AXIS.length; j++){
            ...
            const currentPiece = activePiece ? pieces.find((p) => {
                return isSamePosition(p.position, grabPosition);
            }) : undefined;
            let highlight = currentPiece?.possibleMoves ? currentPiece.possibleMoves.some(p => isSamePosition(p, {x:j+1, y:i+1})) : false;
            board.push(<Tile num={key} flag={flag} image={pieceImg} key={key} highlight={highlight} />);
        }
    }
    ...
    const grabElement = (e)=>{
        ...
        if(element.classList.contains('chessPiece') && currentBoard){
            console.log((e.clientY - currentBoard.offsetTop - 600)/75);
